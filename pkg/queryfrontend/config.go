...
type MemcachedResponseCacheConfig struct {
	...
	// TTL is an accepted alias for Expiration, matching the name the store caches use.
...
func resolveCacheExpiration(backendConfig []byte) (*time.Duration, error) {
	...
	expirationSet := probe.Expiration != nil && *probe.Expiration != 0
	ttlSet := probe.TTL != nil && *probe.TTL != 0
	if expirationSet && ttlSet {
